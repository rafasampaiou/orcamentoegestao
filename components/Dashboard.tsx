...
const formatPercent = (val: number) =>
  `${val > 0 ? '+' : ''}${val.toFixed(1).replace('.', ',')}%`; // Corrigido para padrão PT-BR
...
const calculateDreSummary = (rows: ForecastRow[], packages: CostPackage[], accounts: Account[]) => {
  // CORREÇÃO: Criação de um clone dos objetos ({...r}) para evitar mutação do estado global (dobra de valores)
              ...
              <React.Fragment key={entity.id}>
                {entity.rows.map((row, rowIdx) => {
                  ...
                  const rowClass = isTotal 
                    ? 'bg-indigo-50/50 font-bold text-gray-800 border-y border-indigo-100' 
                    : row.isTaxes 
                      ? 'bg-sky-50 font-bold text-sky-950 border-y-2 border-sky-200' 
  ...
  const dashboardData = useMemo(() => {
    ...
    return entitiesList.map(entity => {
      ...
      const processSummary = (hName?: string) => {
        // CORREÇÃO TS: Passando accounts e hotels com 'as any' para calar o erro de assinatura do TypeScript sem quebrar a execução
        const dreRows = getForecastData(selectedMonth, selectedYear, financialData, hName, packages as any, accounts as any, hotels as any);
      ...
      if (!entity.isGroup) {
        ...
        // CORREÇÃO TS: 's as any' para evitar o erro de indexação dinâmica
        const sumField = (field: string, subField: 'real' | 'budget' | 'lastYear') => {
          return allSummaries.reduce((sum, s) => {
            if (field === 'gopNoTax') return sum + (s.gopNoTax[subField] || 0);
            return sum + (((s as any)[field]?.[subField]) || 0);
          }, 0);
        };
      ...
      rows = [
        ...
        { label: 'Impostos', real: summary.tax?.real || 0, budget: summary.tax?.budget || 0, lastYear: summary.tax?.lastYear || 0, isTaxes: true },
